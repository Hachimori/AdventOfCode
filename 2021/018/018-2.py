...
def work(snailfishNumberList):
    ...
    for i in range(len(snailfishNumberList)):
        for j in range(len(snailfishNumberList)):
            if i == j: continue

            toProcess = ['['] + snailfishNumberList[i] + snailfishNumberList[j] + [']']
            process(toProcess)
            maxV = max(maxV, calcMagnitude(toProcess))
